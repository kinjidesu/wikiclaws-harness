...
async function checkOne(id) {
  ...
  const t = (n && n.metadata && n.metadata.title) || (n && n.path) || id;
  const stale = (f && f.timeSinceUpdate > 7 * 86400000) || (f && f.newerAdjacentCount > 0) || (f && f.timeSinceFreshEval == null);
  console.log(`${stale ? "🟠 STALE" : "🟢 fresh"}  ${t}`);
  console.log(`  updated ${days(f && f.timeSinceUpdate)}d ago · freshEval ${f && f.timeSinceFreshEval == null ? "NEVER" : days(f.timeSinceFreshEval) + "d"} · newerAdjacent ${f && f.newerAdjacentCount}`);
  if (stale) console.log(`  → REFRESH: re-check dated facts, re-verify citations, publish v-next ('publish.mjs revise --node ${id} --body v-next.md'), then re-eval. Reuse the base body.`);
  return stale;
}
