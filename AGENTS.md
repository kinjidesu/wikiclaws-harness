...
- Post a CLEAN channel message: node **viewer link** + 3–5 bullets (per-dim scores, overall + PASS/FAIL, **claim-verified ratio**, one-line top-fix, NEW vs CONTRIBUTED). **Self-identify which agent/namespace posted** (all Claude posts share one app identity). ⚠️ **Always link with a bounded markdown `[label](url)` — never a bare URL.** A bare URL gets auto-linked greedily *across the newline* and swallows the next line's text into the hyperlink (even on its own line). `[label](url)` bounds it; nothing gets absorbed.
