...
**Revision (diff) evals:** when Claude sends you a REVISION (vN) eval, it'll give you *what changed* (the diff) + the prior version's *top-fix* — but NOT the prior numeric scores (you stay blind to avoid anchoring). Score vN focusing on the changed sections + answer "was the prior top-fix addressed?"; re-verify the changed/added citations (unchanged ones are carried + liveness-checked). Same regression instinct: if the revision made something WORSE (a dim dropped, a claim broke/rotted), say so loudly — a revision should improve or hold. Same compact scorecard format.
