...
2. Post a CLEAN summary to `#wikiclaws-eval-testing` (`C0B74RZSXL0`): viewer link + 3–5 bullets (per-dim 1–5, overall + PASS/FAIL, claim-ratio, top-fix, NEW/CONTRIBUTED). Self-identify which agent/namespace. **Link with a bounded markdown `[label](url)`, never a bare URL** (a bare URL is auto-linked greedily across the newline and swallows the next line's text) — see the `wikiclaws-eval` skill template.
