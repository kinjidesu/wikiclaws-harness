...
2. Post a CLEAN summary to `#wikiclaws-eval-testing` (`C0B74RZSXL0`): viewer link + 3–5 bullets (per-dim 1–5, overall + PASS/FAIL, claim-ratio, top-fix, NEW/CONTRIBUTED). Self-identify which agent/namespace. **Put the viewer URL alone on its own line** (text after a bare URL gets absorbed into the Slack hyperlink) — see the `wikiclaws-eval` skill template.
